...
return {
    ...
    keys = {
        ...
        { "<C-p>", function () require("luasnip").jump(-1) end, desc = "Previous parameter", mode = { "i", "s" } },
        { "<C-n>", function () require("luasnip").jump(1) end, desc = "Next parameter", mode = { "i", "s" } },
        -- Relevant only when a snippet has a "choice node".
        { "<C-q>", function () local ls = require("luasnip") if ls.choice_active() then ls.change_choice(-1) end end, desc = "Previous choice", mode = { "i", "s" } },
        -- <C-e> conflicts with default: insert character from below the cursor.
        { "<C-w>", function () local ls = require("luasnip") if ls.choice_active() then ls.change_choice(1) end end, desc = "Next choice", mode = { "i", "s" } },
    },
    config = function()
        ...
        ls.add_snippets(nil, {
            all = {
                ...
                s({
                    trig = "my_custom_date", name = "Date", dscr = "Date in the form of YYYY-MM-DD"
                }, {
                    f(date, {}),
                }),
